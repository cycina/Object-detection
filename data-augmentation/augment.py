...
def imagedrehen(pathtoimage,pathout, name):
    image = cv2.imread(pathtoimage)
    for angle in anglelist:
        rotated = imutils.rotate_bound(image, angle)
        cv2.waitKey(0)
        cv2.imwrite(pathout+name[:-4] + '-' + str(angle) + '.jpg', rotated)
        print('image',name,'rotated',angle,'°')

def xmldrehen(pathtoxml,pathout, xmlname):
    old = open(pathtoxml, "r").readlines()
    for angle in anglelist:
        newxmlname = xmlname[:-4] + '-' + str(angle) + '.xml'
        print('XMLfile ' + xmlname + ' rotated ' + str(angle) + '°')
        new = open(pathout+newxmlname, 'w')
        # no rotation
        if angle == 0:
            for line in old:
                if '<folder>' in line:
...
def imagemirror(pathtoimage,pathout, name):
    image = cv2.imread(pathtoimage)
    rotated = cv2.flip(image, flipCode=1)
    cv2.waitKey(0)
    cv2.imwrite(pathout + name[:-4] + '-' + 'mirrored' + '.jpg', rotated)
    print('image', name, ' mirrored')

def xmlmirror(pathtoxml,pathout, xmlname):
    old = open(pathtoxml, "r").readlines()
...
outTest ='images/test/'
outTrain='images/train/'
testdata='images/testdata/'
traindata='images/traindata/'
#train images
# rotate images and xml
for file in os.listdir(traindata):
    if file.endswith('.jpg' or '.JPG' or '.png' or '.PNG'):
        imagedrehen(traindata+file, outTrain, file)
    if file.endswith('.xml'):
        xmldrehen(traindata+file,outTrain, file)

# mirror images and xml
for file in os.listdir(outTrain):
    if file.endswith('.jpg' or '.JPG' or '.png' or '.PNG'):
        imagemirror(outTrain+file,outTrain, file)
    if file.endswith('.xml'):
        xmlmirror(outTrain+file,outTrain, file)
#testImage
# rotate images and xml
for file in os.listdir(testdata):
    if file.endswith('.jpg' or '.JPG' or '.png' or '.PNG'):
        imagedrehen(testdata+file,outTest, file)
    if file.endswith('.xml'):
        xmldrehen(testdata+file,outTest, file)

# mirror images and xml
for file in os.listdir(outTest):
    if file.endswith('.jpg' or '.JPG' or '.png' or '.PNG'):
        imagemirror(outTest+file,outTest, file)
    if file.endswith('.xml'):
        xmlmirror(outTest+file,outTest, file)


print('The dataset is now augmented by rotating and mirroring it.')
