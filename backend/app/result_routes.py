...
def get_results():
    if request.method == 'GET':
        ...
        for res in results:
            result_data = {}
            # order_data = {}
            # order_data['orderId'] = res.order.orderId
            # order_data['date'] = res.order.date

            result_data['resultId'] = res.resultId
            result_data['state'] = res.state
            result_data['description'] = res.description
            result_data['title'] = res.title
            result_data['image'] = res.image
...
@app.route("/result/<string:productId>", methods=['DELETE'])
def delete_result(productId):
    product = Result.query.get_or_404(productId)
...
def delete_all_results():
    products = Result.query.all()
    for product in products:
        db.session.delete(product)
        db.session.commit()
    return jsonify(' results has been deleted!', 'success')


@app.route("/result/<int:resultId>", methods=['GET'])
def get_one_result(resultId):
    ...
    if result is None:
        abort(
            404,
            "Product not found for Id: {resultId}".format(
                resultId=resultId),
        )
    else:
        result = []
        result_data = {}
        # order_data = {}
        # order_data['orderId'] = result.order.orderId
        # order_data['date'] = result.order.date

        result_data['resultId'] = result.resultId
        result_data['state'] = result.state
...
def detect():
    ...
    graphname = os.path.join('app/data', 'frozen_inference_graph.pb')
    ...
    cv2.imwrite('app/data/test.jpg', image)
    img = cv2.imread('app/data/test.jpg')
...
def detect_all():
    ...
    for file in os.listdir('app/orders/'):
        if file.endswith('.jpg' or '.JPG' or '.png' or '.PNG'):
            ...
            cv2.imwrite('app/data/thr.jpg', img)
            ...
            if final_state==True:
                ...
                if len(notinorder) > 0 or len(more):
                    ...
                    if len(more) > 0:
                        for item in more:
                            description +="(%s) %s \n" % (item['quantity'], item['product'])
            if(order.result_id is None):
                new_result = Result(state=final_state,
                                    description=description, title=order.orderId,
                                    image= image64.decode('ascii'))            
                db.session.add(new_result)
                db.session.commit()
                order.result_id = new_result.resultId
                db.session.commit()
            else:
                update_result = Result.query.filter(
                    Result.resultId == order.result_id
                ).one_or_none()
                update_result.description=description
                update_result.image= image64.decode('ascii')
                update_result.state=final_state
                db.session.commit()
    return jsonify("check orders have been successfully completed")
